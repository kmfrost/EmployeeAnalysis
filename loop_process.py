...
def main():
    ...
    for this_run in xrange(num_runs):
        np.random.seed(this_run)
        print("Run number: {0}".format(this_run))
...
#    
    ...
    predicted_probs = np.mean(all_probs,1)
    thresh = np.mean(all_threshes)
    pkl.dump((all_probs, all_threshes), open('all_runs.pkl', 'wb'))

    # write the results
    res = pd.read_csv('../results.csv')
    res = res.assign(Attrition=predicted_probs)

    thresh_bump = 0.0
    res['Attrition'] = pd.Series(np.where(res.Attrition.values > 1-thresh, 'No', 'Yes'),
