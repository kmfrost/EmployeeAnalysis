...
#                            bootstrap_features=True)
    ...
    classifiers = [SVC_clf, GP_clf, AB_clf, MLP_clf]
    
    num_outer = 10 # change random seed each time
    num_inner = 5  # redo train/test split
    clf_thresh = []
    clf_probs = []
    clf_predictions = []
    ...
    for outer_loop in xrange(num_outer):
        ...
        for inner_loop in xrange(num_inner):
            ...
            for clf in classifiers:
                ...
                test_probs = clf.predict_proba(test_data[:,0])
    ...
    final_prediction = np.mean(clf_probs, 1)
    ...
    res['Attrition'] = pd.Series(np.where(res.Attrition.values > 1-thresh + thresh_bump, 'No', 'Yes'),
                       res.Attrition.index)
    res.to_csv('../SVC_AB_MLP_KNN_Probs.csv', index=False)
